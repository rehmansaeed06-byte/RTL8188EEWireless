...
/*
 * __skb_dequeue() / __skb_queue_purge() -- lockless-internal counterparts
 * of the already-existing skb_dequeue()/skb_queue_purge(), same pattern
 * as __skb_queue_tail()/__skb_unlink() immediately above: caller already
 * holds list->lock. Real rtlwifi call sites (core.c, pci.c,
 * rtl8188ee/hw.c -- confirmed via grep against only compiled files) use
 * these inside sections already under the queue's own lock, so a
 * locking variant here would double-lock.
 */
...
/*
 * skb_queue_is_last() -- real upstream helper, genuinely missing.
 * True if skb is the last element in list.
 */
